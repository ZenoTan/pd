...
func newEmptyRegion() *Case {
	...
	for i := 0; i < getRegionNum(); i++ {
		if i < getRegionNum() / 10 {
		...
		} else if i < 9 * getRegionNum() / 10 {
			storeIDs := rand.Perm(9)
			peers := []*metapb.Peer{
				{Id: IDAllocator.nextID(), StoreId: uint64(storeIDs[0] + 1)},
				{Id: IDAllocator.nextID(), StoreId: uint64(storeIDs[1] + 1)},
				{Id: IDAllocator.nextID(), StoreId: uint64(storeIDs[2] + 1)},
			...
			storeIDs := rand.Perm(9)
			peers := []*metapb.Peer{
				{Id: IDAllocator.nextID(), StoreId: 10},
				{Id: IDAllocator.nextID(), StoreId: uint64(storeIDs[0] + 1)},
				{Id: IDAllocator.nextID(), StoreId: uint64(storeIDs[1] + 1)},
			}
			simCase.Regions = append(simCase.Regions, Region{
	...
	table10 := string(codec.EncodeBytes(codec.GenerateTableKey(10)))
	table11 := string(codec.EncodeBytes(codec.GenerateTableKey(11)))
	...
	simCase.Checker = func(regions *core.RegionsInfo, stats []info.StoreStats) bool {
		...
		regions.ScanRangeWithIterator([]byte(table10), func(region *core.RegionInfo) bool {
			if bytes.Compare(region.GetEndKey(), []byte(table11)) < 0 {
